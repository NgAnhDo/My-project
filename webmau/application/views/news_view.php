			...
			<div class="row">
				<div class="col-sm-9">
					...
					<div class="mottinchuan mb-3  wow  fadeInUp">
						...
						<a href="<?= base_url(); ?>news/chiTietTin/<?= $value['id'] ?>"><img src="<?= $value['anhtin'] ?>" alt=""></a>
						<a href="<?= base_url(); ?>news/chiTietTin/<?= $value['id'] ?>" class="tieudetin1 fontoswarld"><?= $value['tieude'] ?></a>
						<div class="ngaythang1"><?= $value['ngaytao'] ?> <span class="vang"> <?= $value['tendanhmuc'] ?></span></div>
						<p class="fontroboto"><?= $value['trichdan'] ?></p>

						<div class="docthem mb-2">
							
							<a href="<?= base_url(); ?>news/chiTietTin/<?= $value['id'] ?>" class="rm fontroboto">Read More</a>
						</div>
					</div>
					...
					<nav class="phantrang mb-3  wow  fadeInUp">
						<ul class="pagination">
							<?php if ($tranghientai==1) {
								...
								<li class="pre">
								<a href="<?= base_url() ?>news/page/<?= $tranghientai-1 ?>" aria-label="Previous">
							...
							<?php for ($i = 0; $i <$sotrang ; $i++) { ?>
								...
								<li ><a href="<?= base_url() ?>news/page/<?= $i +1?>"  ><?= $i +1?></a></li>
							<?php } ?>
							<?php } ?>
					...
					<div class="khoilistlink my-2  wow  fadeInUp">
						...
						<ul class="fontroboto">
							<?php foreach ($cacdanhmuc as $value): ?>
								<li><a href="<?= base_url() ?>news/danhMuc/<?= $value['id'] ?>"> <?= $value['tendanhmuc'] ?></a></li>
							<?php endforeach ?>
